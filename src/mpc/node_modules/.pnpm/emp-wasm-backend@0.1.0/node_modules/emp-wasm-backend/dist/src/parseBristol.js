export default function parseBristol(input) {
    const lines = input
        .trim()
        .split(/\r?\n/)
        .map((line) => line.trim())
    ...
    if (lines.length !== 3 + numGates) {
        throw new Error(`Expected ${numGates} gates, but found ${lines.length - 3}.`);
    }
    ...
    for (let i = 3; i < lines.length; i++) {
        ...
        if (isNaN(numGateInputs) || isNaN(numGateOutputs)) {
            throw new Error(`Invalid number of inputs or outputs at line ${i + 1}: "${line}"`);
        }
        const expectedTokens = 2 + numGateInputs + numGateOutputs + 1;
        if (tokens.length !== expectedTokens) {
            throw new Error(`Expected ${expectedTokens} tokens but found ${tokens.length} at line ${i + 1}: "${line}"`);
        }
        ...
        if (inputWires.some((w) => isNaN(w) || w < 0 || w >= wireCount) ||
            outputWires.some((w) => isNaN(w) || w < 0 || w >= wireCount)) {
            throw new Error(`Invalid wire indices at line ${i + 1}: "${line}"`);
        }
        switch (gateType) {
            case 'AND':
            case 'XOR':
            case 'OR':
                if (numGateInputs !== 2 || numGateOutputs !== 1) {
                    throw new Error(`Invalid inputs/outputs for ${gateType} at line ${i + 1}: "${line}"`);
                }
                gates.push({
                    type: gateType,
                    left: inputWires[0],
            ...
            case 'COPY':
                ...
                break;
            default:
                throw new Error(`Unknown gate type '${gateType}' at line ${i + 1}: "${line}"`);
        }
    }
    return {
        wireCount,
...
//# sourceMappingURL=parseBristol.js.map