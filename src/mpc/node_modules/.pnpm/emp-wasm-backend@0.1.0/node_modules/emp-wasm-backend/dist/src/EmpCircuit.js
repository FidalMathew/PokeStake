import parseBristol from "./parseBristol";
...
export default class EmpCircuit {
    ...
    nextOutputWireId = -1;
    zeroWireId;
    constructor(circuit, mpcSettings) {
        this.bristol = parseBristol(circuit.bristol);
        this.info = circuit.info;
        assert(mpcSettings.length === 2, 'Expected exactly two participants');
        this.aliceInputs = mpcSettings[0].inputs;
        this.aliceInputs.sort((a, b) => circuit.info.input_name_to_wire_index[a] -
            circuit.info.input_name_to_wire_index[b]);
        this.bobInputs = mpcSettings[1].inputs;
        this.bobInputs.sort((a, b) => circuit.info.input_name_to_wire_index[a] -
            circuit.info.input_name_to_wire_index[b]);
        this.allInputs = [...this.aliceInputs, ...this.bobInputs];
        this.allInputs.sort((a, b) => circuit.info.input_name_to_wire_index[a] -
            circuit.info.input_name_to_wire_index[b]);
        this.outputs = mpcSettings[0].outputs;
        this.outputs.sort((a, b) => circuit.info.output_name_to_wire_index[a] -
        ...
        for (const g of this.bristol.gates) {
            ...
            switch (g.type) {
                ...
                case 'OR': {
                    ...
                    outputWireId = this.assignWireId(wireType);
                    this.gates.push({ type: 'INV', input: this.getWireId(g.left), output: notA }, { type: 'INV', input: this.getWireId(g.right), output: notB }, { type: 'AND', left: notA, right: notB, output: notAAndNotB }, { type: 'INV', input: notAAndNotB, output: outputWireId });
                    break;
                }
                case 'COPY': {
        ...
        const reassignOutputWireId = (wireId) => {
            assert(wireId < 0);
    ...
    encodeInput(party, input) {
        const inputNames = party === 'alice' ? this.aliceInputs : this.bobInputs;
        const bits = [];
        for (const inputName of inputNames) {
            const value = input[inputName];
            const width = this.getInputWidth(inputName);
            assert(typeof value === 'number', `Expected input ${inputName} to be a number`);
...
//# sourceMappingURL=EmpCircuit.js.map