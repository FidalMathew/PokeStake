...
windowAny.internalDemo = async function (aliceInput, bobInput) {
    const aliceBq = new BufferQueue();
    const bobBq = new BufferQueue();
    const [aliceBits, bobBits] = await Promise.all([
        secure2PC('alice', add32BitCircuit, numberTo32Bits(aliceInput), {
            send: data => bobBq.push(data),
            recv: len => aliceBq.pop(len),
        }),
        secure2PC('bob', add32BitCircuit, numberTo32Bits(bobInput), {
...
windowAny.consoleDemo = async function (party, input) {
    const bits = await secure2PC(party, add32BitCircuit, numberTo32Bits(input), makeCopyPasteIO());
    alert(numberFrom32Bits(bits));
};
windowAny.wsDemo = async function (party, input) {
    const io = await makeWebSocketIO('ws://localhost:8175/demo');
    const bits = await secure2PC(party, add32BitCircuit, numberTo32Bits(input), io);
    io.close();
    return numberFrom32Bits(bits);
};
windowAny.rtcDemo = async function (pairingCode, party, input) {
    const io = await makePeerIO(pairingCode, party);
    const bits = await secure2PC(party, add32BitCircuit, numberTo32Bits(input), io);
    io.close();
    return numberFrom32Bits(bits);
};
async function makeWebSocketIO(url) {
    ...
    // bytes
    const io = new BufferedIO(data => sock.send(data), () => sock.close());
...
async function makePeerIO(pairingCode, party) {
    ...
    const io = new BufferedIO((data) => conn.send(data), () => peer.destroy());
    conn.on('data', (data) => {
        let buf;
