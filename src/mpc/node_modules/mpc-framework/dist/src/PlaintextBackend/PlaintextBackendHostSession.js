...
export default class PlaintextBackendHostSession {
    circuit;
    mpcSettings;
    name;
    input;
    send;
    outputPromise;
    combinedInputs = defer();
    partialCombinedInputs;
    peerInputsReceived = new Set();
    constructor(circuit, mpcSettings, name, input, send) {
    ...
    handleMessage(from, msg) {
        try {
            ...
            const peerInfo = this.mpcSettings.find((s, i) => from === (s.name ?? i.toString()));
            ...
            this.send(from, pack({ error: `Couldn't handle message: ${errorToString(e)}` }));
        }
    }
    output() {
...
//# sourceMappingURL=PlaintextBackendHostSession.js.map